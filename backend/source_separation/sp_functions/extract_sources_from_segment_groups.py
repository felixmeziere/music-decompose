...
def extract_sources_from_segment_groups(method, segment_groups, segment_STFTs, hop_length, win_length):
    ...
    if method == 'classic':
        for segment_group in segment_groups:
            ...
            rest_segment_STFTs, source_segment_STFTs = _extract_group_source(
                segment_STFTs, extracted_indices, segment_group)
            ...
            sources_segment_WFs.append(STFTs_to_WFs(
                source_segment_STFTs, hop_length, win_length))
        sources_segment_STFTs.append(segment_STFTs)
        sources_segment_WFs.append(STFTs_to_WFs(
            segment_STFTs, hop_length, win_length))
        source_WFs = [np.concatenate(source_segment_WFs)
                      for source_segment_WFs in sources_segment_WFs]
...
def _extract_group_source(
    ...
    if isinstance(group_extracting, int) or len(group_extracting) == 1:
        group_extracting_max_spectro = np.abs(
            segment_STFTs[group_extracting, :, :])
    else:
        group_extracting_max_spectro = np.max(
            np.abs(segment_STFTs[group_extracting, :, :]), axis=0)
    ...
    min_matrix = np.array([np.minimum(source_median_spectro, source_spectros[i, :, :])
                           for i in np.arange(source_spectros.shape[0])])
    ...
    rest_segment_STFTs = np.add(
        rest_segment_STFTs, additional_rest_segment_STFTs)
